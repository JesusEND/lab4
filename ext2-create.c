...
//Defining my own Macros for Super Block funciton
#define NO_LIMIT_MNT_COUNT -1
#define INTERVAL_CHECK 1
#define FS_STATE_VALID 1
#define FS_ERR_BEHAVIOR_CONTINUE 1
#define OS_TYPE_EXT2_LINUX 0
#define DEFAULT_RESUID 0
#define DEFAULT_RESGID 0
#define COUNT_DIRS_IN_BG 2
...
void write_superblock(int fd) {
	...
	superblock.s_max_mnt_count     = NO_LIMIT_MNT_COUNT; /* Make this unlimited */
	superblock.s_magic = EXT2_SUPER_MAGIC; /* ext2 Signature */
	superblock.s_state             = FS_STATE_VALID; /* File system is clean */
	superblock.s_errors            = FS_ERR_BEHAVIOR_CONTINUE; /* Ignore the error (continue on) */
	superblock.s_minor_rev_level   = 0; /* Leave this as 0 */
	superblock.s_lastcheck = current_time; /* Last check time */
	superblock.s_checkinterval     = INTERVAL_CHECK; /* Force checks by making them every 1 second */
	superblock.s_creator_os        = OS_TYPE_EXT2_LINUX; /* Linux */
	superblock.s_rev_level         = 0; /* Leave this as 0 */
	superblock.s_def_resuid        = DEFAULT_RESUID; /* root */
	superblock.s_def_resgid        = DEFAULT_RESGID; /* root */
...
void write_block_group_descriptor_table(int fd) {
	...
	block_group_descriptor.bg_used_dirs_count = COUNT_DIRS_IN_BG;
...
void write_block_bitmap(int fd)
{
	...
	// Fill the remainder of the bitmap (from byte 128 to the end) indicating all blocks are in use, serves as padding
	for(int x = 128; x < BLOCK_SIZE; x++){
		map_value[x] = 0xFF;
	} 
...
void write_inode_bitmap(int fd)
{
	...
	map_value[0] = 0xFF;
	map_value[2] = 0x1F;
	//serves as padding for the bitmap
	for(int x = NUM_INODES / 8; x < BLOCK_SIZE; x++){
		map_value[x] = 0xFF;
	}
	if (write(fd, map_value, BLOCK_SIZE) != BLOCK_SIZE){
		errno_exit("write");
...
void write_inode_table(int fd) {
	...
	//Replicated above, but for the direct root
	struct ext2_inode inode_direct_root = {0};
	inode_direct_root.i_mode = EXT2_S_IFDIR
								| EXT2_S_IRUSR
								| EXT2_S_IWUSR
								| EXT2_S_IXUSR
								| EXT2_S_IRGRP
								...
								| EXT2_S_IROTH
								| EXT2_S_IXOTH;
	inode_direct_root.i_uid = 0;
	inode_direct_root.i_size = 1024;
	inode_direct_root.i_atime = current_time;
	inode_direct_root.i_ctime = current_time;
	inode_direct_root.i_mtime = current_time;
	inode_direct_root.i_dtime = 0;
	inode_direct_root.i_gid = 0;
	...
	hello_world_ino.i_uid = 1000;
	hello_world_ino.i_size = 12; 
	hello_world_ino.i_atime = current_time;
	hello_world_ino.i_ctime = current_time;
	hello_world_ino.i_mtime = current_time;
	hello_world_ino.i_dtime = 0;
	hello_world_ino.i_gid = 1000;
	hello_world_ino.i_links_count = 1;
	hello_world_ino.i_blocks = 2;
	hello_world_ino.i_block[0] = HELLO_WORLD_FILE_BLOCKNO;
	write_inode(fd, HELLO_WORLD_INO, &hello_world_ino);
	//Replicated it for Hello symbolic link
	struct ext2_inode hello_ino = {0};
	hello_ino.i_mode = EXT2_S_IFLNK
							| EXT2_S_IRUSR
						 	| EXT2_S_IWUSR
						 	| EXT2_S_IRGRP
						 	| EXT2_S_IROTH;
	...
	memcpy((char*)hello_ino.i_block, file_path, 12); 
...
{
	//Replicating what is done in write_lost_and_found_dir_block 
	...
	//replicating for a parent entry
	...
	//Lost_and_found_entry
	struct ext2_dir_entry lost_and_found_dir_block_entry = {0};
	dir_entry_set(lost_and_found_dir_block_entry, LOST_AND_FOUND_INO, "lost+found");
	dir_entry_write(lost_and_found_dir_block_entry, fd);
	bytes_remaining -= lost_and_found_dir_block_entry.rec_len;
	//Hello_world entry file
	...
	//symbolic link hello which points to hello_world
	struct ext2_dir_entry hello_link_entry = {0};
	dir_entry_set(hello_link_entry, HELLO_INO, "hello");
	dir_entry_write(hello_link_entry, fd);
	bytes_remaining -= hello_link_entry.rec_len;
